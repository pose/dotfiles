...
if typeset -f whereami > /dev/null; then
  _USERNAME="$_USERNAME%{$reset_color%}@$(whereami)"
else
...
bureau_precmd () {
  _PATH="%{$fg_bold[white]%}$(shrink_path -f)%{$reset_color%}"
...
setopt prompt_subst
PROMPT='> $_LIBERTY '
RPROMPT='$(pose_theme_custom_prompts) $(nvm_prompt_info) $(bureau_git_prompt)'
